...
export interface StripedSludgeDelayParams {
  numVoices: number;
  speed: number;
  range: number;
  directionUp: boolean;
...
export function stripedSludgeDelayGraph(
  params: StripedSludgeDelayParams,
  sampleRate: number
): NodeRepr_t {
  ...
  const smoothFeedback = el.sm(
    el.const({ key: "sld-feedback", value: feedback })
  );

  const smoothGlobalFb = el.sm(
    el.const({ key: "sld-gfb", value: globalFeedback })
  );

  const fbSignal = el.tapIn({ name: "sld-fb" }) as NodeRepr_t;
  const globalFbSignal = el.tapIn({ name: "sld-gfb-tap" }) as NodeRepr_t;
  ...
  const fbHead = el.delay(
    ...
    el.sm(el.const({ key: "sld-fb-delay", value: fbDelaySamples })),
    0,
    toBuffer
  );
  const fbTapped = el.tapOut({ name: "sld-fb" }, fbHead) as NodeRepr_t;
  const fbSink = el.mul(fbTapped, 0) as NodeRepr_t;

  const smoothSpeed = el.sm(
    el.const({ key: "sld-speed", value: speed })
  );
  const smoothRange = el.sm(
    el.const({ key: "sld-range", value: rangeSamples })
  );

  const voiceGain = 2 / numVoices;

  const dir = el.sm(
    el.const({ key: "sld-dir", value: directionUp ? 1.0 : 0.0 })
  );
  ...
  function voice(index: number) {
    ...
    const shifted = el.add(
      raw,
      el.const({ key: `sld-ph-${index}`, value: phaseOffset })
    );
    const phasor = el.sub(shifted, el.floor(shifted));

    // CENTERED HUMP: delay rises then falls (or vice versa)
    // hump = sin²(π×p) = 0.5×(1 - cos(2π×p))
    //   p=0: 0, p=0.5: 1 (peak), p=1: 0
    // For rising pitch: delay humps up then down
    //   → first half: delay increasing → read head slows → pitch BELOW original
    //   → second half: delay decreasing → read head speeds up → pitch ABOVE original
    // For falling pitch: inverted hump
    const hump = el.mul(
      0.5,
      el.sub(1.0, el.cos(el.mul(2 * Math.PI, phasor)))
    );
    const invHump = el.sub(1.0, hump);
    const curve = el.add(
      el.mul(dir, hump),
      el.mul(invDir, invHump)
    );
    ...
    // Hann window with tilt/skew
    const smoothSkew = el.sm(
      el.const({ key: "sld-skew", value: skew })
    );
  ...
  const mixedTapped = el.tapOut({ name: "sld-gfb-tap" }, mixed) as NodeRepr_t;

  const dry = el.mul(
    input,
    el.sm(el.const({ key: "sld-dry", value: 1 - dryWet }))
  ) as NodeRepr_t;
  const wet = el.mul(
    mixedTapped,
    el.sm(el.const({ key: "sld-wet", value: dryWet }))
  ) as NodeRepr_t;
